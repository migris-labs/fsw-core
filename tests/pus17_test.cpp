...
std::vector<std::uint8_t> build_tc_are_you_alive(std::uint16_t source_id, std::uint16_t seq_count) {
    ...
    EXPECT_EQ(migris_pus_tc_secondary_pack(&tc_sec,
...
TEST(Pus17, ValidTcProducesWellFormedTm) {
    migris_pus17_ctx_t ctx = {test_apid, /*tm_seq_count=*/0, /*tm_msg_counter=*/0};
...
TEST(Pus17, ContextCountersAdvanceAndWrap) {
    migris_pus17_ctx_t ctx = {test_apid, /*tm_seq_count=*/0x3FFEU, /*tm_msg_counter=*/0xFEU};
    std::array<std::uint8_t, MIGRIS_PUS17_TM_PACKET_SIZE> tm{};

    for (int i = 0; i < 4; ++i) {
        const auto tc = build_tc_are_you_alive(0, static_cast<std::uint16_t>(i));
        const int rc =
            migris_pus17_handle_are_you_alive(&ctx, 0U, tc.data(), tc.size(), tm.data(), tm.size());
        ASSERT_EQ(rc, static_cast<int>(MIGRIS_PUS17_TM_PACKET_SIZE));
    }
...
TEST(Pus17, RejectsTruncatedTc) {
    migris_pus17_ctx_t ctx = {test_apid, 0, 0};
...
TEST(Pus17, RejectsWrongApid) {
    migris_pus17_ctx_t ctx = {/*apid=*/test_apid + 1U, 0, 0};
    auto tc = build_tc_are_you_alive(0, 0);
