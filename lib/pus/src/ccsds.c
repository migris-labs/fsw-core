...
int migris_ccsds_primary_pack(const migris_ccsds_primary_header_t *hdr,
    ...
    if (hdr->version > 0x7U || hdr->type > 0x1U || hdr->sec_hdr_flag > 0x1U
        || hdr->apid > 0x7FFU || hdr->seq_flags > 0x3U || hdr->seq_count > 0x3FFFU) {
    ...
    const uint16_t word1 = (uint16_t)((uint16_t)(hdr->seq_flags & 0x3U) << 14)
